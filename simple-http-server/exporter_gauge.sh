...
#
#   - gauge_array (array, optional, default: empty array)
#     Added metrics will be appended to this initial array
#     Example: gauge_array=("my_metric1 1.31" "my_metric2 90.0")
...
#
function exporter_add_gauge() {
	if [ -z "${gauge_array+x}" ]; then
	  gauge_array=()
	fi
	...
	if [ -n "$metric_description" ]; then
		gauge_array+=("# HELP $metric_name $metric_description")
	fi
	# silently skip unrecognized $metric_type
	if [ "$metric_type" == "gauge" ]; then
		gauge_array+=("# TYPE $metric_name $metric_type")
	fi
	shift 3
	while (( "$#" )); do
		local metric_labels=()
		...
		if (( "$#" )); then
			...
			gauge_array+=("$metric_name{$metric_labels_line} $metric_value")
		else
			gauge_array+=("$metric_name $metric_value")
		fi
	done
}

function exporter_show_gauge() {
	printf "%s\n" "${gauge_array[@]}"
}
